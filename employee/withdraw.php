...
$accounts_sql = "SELECT a.account_id, a.account_number, a.account_name, a.balance, u.full_name FROM accounts a JOIN users u ON a.user_id = u.user_id WHERE a.status = 'active' ORDER BY a.account_number";
    ...
    if ($account_id <= 0) {
        ...
        if (!$account) {
            ...
            try {
                ...
                $insert_transaction_sql = "INSERT INTO transactions (transaction_reference, from_account_id, to_account_id, transaction_type, amount, description, status, processed_at, processed_by) VALUES (?, NULL, NULL, 'withdrawal', ?, ?, 'completed', NOW(), ?)";
                $stmt = executeQuery($insert_transaction_sql, [$transaction_reference, $amount, $description, $_SESSION['user_id']]);
                
                // Commit transaction
                $conn->commit();
                
                $_SESSION['success'] = 'Withdrawal of $' . number_format($amount, 2) . ' from account ' . $account['account_number'] . ' was successful.';
    ...
    <aside class="dashboard-sidebar">
        ...
        <nav class="dashboard-sidebar-menu">
            <ul>
                ...
                <li><a href="/employee/withdraw.php" class="active"><i class="fas fa-arrow-up"></i> Withdraw</a></li>
