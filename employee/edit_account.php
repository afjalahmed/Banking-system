...
if ($account_id <= 0) {
    header('Location: /employee/accounts.php?error=Invalid account ID');
    exit;
}

// Fetch account data
$account_sql = "SELECT a.*, u.full_name, u.email FROM accounts a 
...
if (!$account) {
    header('Location: /employee/accounts.php?error=Account not found');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        if ($txn_count > 0) {
            ...
            header('Location: /employee/accounts.php?success=Account deleted successfully');
            exit;
        }
    }
}
        ...
        <div class="table-container" style="margin-bottom: 2rem;">
            ...
            <div style="padding: 1.5rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                ...
                <div>
                    ...
                    <div><?php echo htmlspecialchars($account['full_name']); ?></div>
                </div>
                <div>
