    ...
    if ($transaction_id > 0 && in_array($action, ['approve', 'reject'])) {
        ...
        if ($transaction) {
            if ($action === 'approve') {
                ...
                if ($can_approve) {
                    ...
                    try {
                        ...
                        $log_sql = "INSERT INTO audit_logs (user_id, action, table_name, record_id, new_values) 
                                   VALUES (?, ?, ?, ?, ?)";
                        executeQuery($log_sql, [$_SESSION['user_id'], 'ADMIN_APPROVE_TRANSACTION', 
                ...
                $log_sql = "INSERT INTO audit_logs (user_id, action, table_name, record_id, new_values) 
                           VALUES (?, ?, ?, ?, ?)";
                executeQuery($log_sql, [$_SESSION['user_id'], 'ADMIN_REJECT_TRANSACTION', 
...
$type_count_sql = "SELECT transaction_type, COUNT(*) as count, SUM(amount) as total 
                    FROM transactions WHERE status = 'pending' GROUP BY transaction_type";
$type_count_result = executeQuery($type_count_sql);
$type_breakdown = fetchAll($type_count_result);
?>
...
<div class="dashboard-layout">
    ...
    <aside class="dashboard-sidebar">
        ...
        <nav class="dashboard-sidebar-menu">
            <ul>
                ...
                <li><a href="/admin/accounts.php"><i class="fas fa-university"></i> Accounts</a></li>
                <li><a href="/admin/transactions.php"><i class="fas fa-exchange-alt"></i> All Transactions</a></li>
                <li><a href="/admin/pending_transactions.php" class="active"><i class="fas fa-clock"></i> Pending Approval</a></li>
    ...
    <main class="dashboard-content">
        ...
        <div class="stats-grid" style="grid-template-columns: repeat(4, 1fr);">
            ...
            <div class="stat-card">
                <div class="stat-icon <?php echo $type['transaction_type'] === 'deposit' ? 'success' : ($type['transaction_type'] === 'withdrawal' ? 'danger' : 'primary'); ?>">
                    <i class="fas fa-<?php echo $type['transaction_type'] === 'deposit' ? 'arrow-down' : ($type['transaction_type'] === 'withdrawal' ? 'arrow-up' : 'exchange-alt'); ?>"></i>
                </div>
                <div class="stat-info">
                    <h3><?php echo ucfirst($type['transaction_type']); ?>s</h3>
                    <div class="stat-value"><?php echo number_format($type['count']); ?></div>
                    <small>$<?php echo number_format($type['total'], 2); ?></small>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            <form method="POST" style="display: inline;" onsubmit="return confirm('Approve this transaction? This will update account balances.');">
